...
describe("node-fetch", () => {
  ...
  it("should remove internal AbortSignal event listener after request is aborted", function() {
    ...
    const promise = fetch(`${base}timeout`, { signal });
    const result = expect(promise)
      .to.eventually.be.rejected.and.be.an.instanceof(Error)
  ...
  it("should remove internal AbortSignal event listener after request and response complete without aborting", () => {
    const controller = new AbortController();
    const { signal } = controller;
    const fetchHtml = fetch(`${base}html`, { signal }).then(res => res.text());
    const fetchResponseError = fetch(`${base}error/reset`, { signal });
    const fetchRedirect = fetch(`${base}redirect/301`, { signal }).then(res =>
      res.json()
    );
    return Promise.all([
  ...
  (supportStreamDestroy ? it : it.skip)(
    "should cancel request body of type Stream with AbortError when aborted",
    () => {
      const controller = new AbortController();
  ...
  (supportStreamDestroy ? it.skip : it)(
    "should immediately reject when attempting to cancel streamed Requests in node < 8",
    () => {
      const controller = new AbortController();
      const body = new stream.Readable({ objectMode: true });
      body._read = () => {};
  ...
  itUSP(
    "constructing a Response with URLSearchParams as body should have a Content-Type",
    function() {
  ...
  itUSP(
    "constructing a Request with URLSearchParams as body should have a Content-Type",
    function() {
      const params = new URLSearchParams();
      const req = new Request(base, { method: "POST", body: params });
      expect(req.headers.get("Content-Type")).to.equal(
        "application/x-www-form-urlencoded;charset=UTF-8"
      );
    }
  );
  ...
  itUSP(
    "Reading a body with URLSearchParams should echo back the result",
    function() {
      const params = new URLSearchParams();
      params.append("a", "1");
  ...
  itUSP(
    "constructing a Request/Response with URLSearchParams and mutating it should not affected body",
    function() {
      const params = new URLSearchParams();
...
describe("external encoding", () => {
  ...
  describe("with optional `encoding`", function() {
      ...
      return fetch(url).then(res => {
        ...
        return res.text().then(result => {
          expect(result).to.equal(
            '<?xml version="1.0" encoding="EUC-JP"?><title>\ufffd\ufffd\ufffd\u0738\ufffd</title>'
          );
        });
      });
    });
    ...
    it("should support encoding decode, html4 detect", function() {
      ...
      return fetch(url).then(res => {
        ...
        return res.textConverted().then(result => {
          expect(result).to.equal(
            '<meta http-equiv="Content-Type" content="text/html; charset=gb2312"><div>中文</div>'
          );
        });
      });
    });
    ...
    it("should support chunked encoding, html4 detect", function() {
      ...
      return fetch(url).then(res => {
        ...
        return res.textConverted().then(result => {
          expect(result).to.equal(
            `${padding}<meta http-equiv="Content-Type" content="text/html; charset=Shift_JIS" /><div>日本語</div>`
          );
        });
      });
    });
  ...
  describe("without optional `encoding`", function() {
    ...
    it("should throw a FetchError if res.textConverted() is called without `encoding` in require cache", () => {
      const url = `${base}hello`;
      return fetch(url).then(res => {
        return expect(res.textConverted())
          .to.eventually.be.rejected.and.have.property("message")
          .which.includes("encoding");
      });
    });
  });
